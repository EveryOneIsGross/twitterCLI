"""
Twitter CLI - Command line interface for Twitter API operations with direct message functionality that's limited to the premium tier
"""
...
class RateLimitState:
    ...
    def is_limited(self) -> bool:
        ...
        return self.requests_remaining <= 0 and datetime.utcnow() < self.reset_at
...
class TwitterAPI:
    def __init__(self):
        ...
        self._user_id = None
        self._user_id_cache_file = os.path.join(os.path.expanduser('~'), '.twitter_user_id.json')
    ...
    def get_dm_events(self, limit: int = 50) -> Dict[str, Any]:
        """Get all DM events for the authenticated user"""
        if not self._check_dm_permissions():
            return {
                "error": (
                    "Direct Message access not authorized. Please update your app permissions:\n"
                    "1. Go to Twitter Developer Portal\n"
                    "2. Select your App\n"
                    "3. Go to 'User authentication settings'\n"
                    "4. Change 'App permissions' to 'Read and write and Direct message'\n"
                    "5. Save changes\n"
                    "6. Run 'python twitterCLI.py reset-cache' to re-authenticate"
                )
            }
            
        print(f"\n💬 Fetching DM events (limit: {limit})")
        
        params = {
            "max_results": min(limit, 100),
            "dm_event.fields": "id,text,created_at,dm_conversation_id,sender_id,referenced_tweets",
            "expansions": "sender_id,referenced_tweets.id",
            "user.fields": "username,name,profile_image_url",
            "tweet.fields": "id,text,created_at"
        }
        
        return self._make_request("get", "2/dm_events", params=params)

    def get_dm_conversation(self, conversation_id: str, limit: int = 50) -> Dict[str, Any]:
        ...
        return self._make_request(
            "get", 
            f"2/dm_conversations/{conversation_id}/dm_events",
            params=params
        )

    def get_dm_conversation_with_user(self, participant_id: str, limit: int = 50) -> Dict[str, Any]:
        """Get DM events for a conversation with a specific user"""
        if not self._check_dm_permissions():
            return {
                "error": (
                    "Direct Message access not authorized. Please update your app permissions:\n"
        ...
        print(f"\n💬 Fetching conversation with user {participant_id}")
        ...
        return self._make_request(
            "get",
            f"2/dm_conversations/with/{participant_id}/dm_events",
            params=params
        )

    def _check_dm_permissions(self) -> bool:
        """Check if the app has DM permissions"""
        try:
            # Use a minimal request to test permissions
    ...
    def format_dm_events(self, result: Dict[str, Any]) -> None:
        """Format and display DM events"""
        if "error" in result:
            print(f"\n❌ Error: {result['error']}")
            return
        ...
        for event in result["data"]:
            ...
            print(
                ...
                f"\n🆔 Conversation: {event.get('dm_conversation_id', 'unknown')}"
                f"\n{'-' * 50}"
            )
        
        if result.get("meta", {}).get("next_token"):
            print("\nℹ️ More messages available. Use --limit to retrieve more.")
...
def initialize_rate_limits() -> Dict[str, EndpointLimits]:
    ...
    # Add DM endpoints rate limits
    rate_limits["GET /2/dm_events"] = EndpointLimits(
        user_limit=RateLimit(rate=300, period=Period(min=15), scope="PER USER")
    )
    
    rate_limits["GET /2/dm_conversations/with/:participant_id/dm_events"] = EndpointLimits(
        user_limit=RateLimit(rate=300, period=Period(min=15), scope="PER USER")
    )
    
    rate_limits["GET /2/dm_conversations/:dm_conversation_id/dm_events"] = EndpointLimits(
        user_limit=RateLimit(rate=300, period=Period(min=15), scope="PER USER")
    )
...
def setup_argparse() -> argparse.ArgumentParser:
    ...
    # DM commands
    dm_parser = subparsers.add_parser('dm', help='Direct Message operations')
    dm_subparsers = dm_parser.add_subparsers(dest='dm_command', help='DM commands')
    
    # List all DMs
    dm_list = dm_subparsers.add_parser('list', help='List all DM events')
    dm_list.add_argument('--limit', type=int, default=50, help='Number of DMs to retrieve')
    
    # Get conversation by ID
    dm_conv = dm_subparsers.add_parser('conversation', help='Get DMs from a specific conversation')
    dm_conv.add_argument('conversation_id', help='Conversation ID')
    dm_conv.add_argument('--limit', type=int, default=50, help='Number of DMs to retrieve')
    
    # Get conversation with user
    dm_with = dm_subparsers.add_parser('with', help='Get DMs with a specific user')
    dm_with.add_argument('user_id', help='User ID to get conversation with')
    dm_with.add_argument('--limit', type=int, default=8, help='Number of DMs to retrieve')
...
def main():
    ...
    try:
        ...
        if args.command == 'user':
            ...
            if not args.dm_command:
                print("❌ Please specify a DM command (list, conversation, or with)")
                return
                
            if args.dm_command == 'list':
                result = twitter.get_dm_events(args.limit)
                twitter.format_dm_events(result)
                
            elif args.dm_command == 'conversation':
                result = twitter.get_dm_conversation(args.conversation_id, args.limit)
                twitter.format_dm_events(result)
                
            elif args.dm_command == 'with':
                result = twitter.get_dm_conversation_with_user(args.user_id, args.limit)
                twitter.format_dm_events(result)
